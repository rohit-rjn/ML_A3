...
% Transform t into one hot encoding
ttrain2 = [];
for i=1: size(ttrain)
    temp = zeros(1,10);
    temp( ttrain(i)) = 1;
    ttrain2 = vertcat(ttrain2, temp);
end
...
for epoch=1:MAX_EPOCH
  ...
  for x_i=1:N
    ...
    % TO DO:: fill this in.
    dW2 = zeros(H+1,K);
    % dW2 = [ones(size(Z{1}, 1), 1) Z{1}]' * (Z{2} - ttrain2(x_i, :) );
    dW2 = Z{1}' * (Z{2} - ttrain2(x_i, :) );
    dW2 = [dW2;zeros(1,K)];
    ...
    dj = (Z{1} .* (1 - Z{1})).*((Z{2} - ttrain2(x_i,:)) * NN(2).weights(1:500,:)'); 
    dW1 = Xtrain(x_i,:)' * dj;
    dW1 = [dW1;zeros(1,H)];
